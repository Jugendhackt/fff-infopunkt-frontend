...
<div class="main">
  ...
  <ul v-if="searched">
    <div v-for="item in items" v-bind:key="item.strikeId">
      <ListItem :date="item.date" :org="item.organisation" :orgurl="item.url" :treffpunkt="item.meetingPoint" :ziel="item.endPoint" :time="item.startTime" :strecke="item.routeLength" :titel="item.title" :beschreibung="item.description" :quelle="item.groupSource"/>
    </div>
  </ul>
</div>
</template>
